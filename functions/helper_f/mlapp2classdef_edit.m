...
if iscell(pathToMLapp)
    ...
    checkfile(pathname, filename, ext); % file exists and .mlapp extension?
  ...
  %  newname=[appname,'M.m'];
    ...
    delete(fullfile(pathname,[appname,'.m']));
  %--------------------------------------------------------
end
...
% [ I  ] Check for existence of file
% [ II ] Check for correct file type
% [else] exit via error
...
% This is the core function which is entered after the optionsflags are
% created
%
%[ I  ]
%[ I  ]
...
if uielementflag.ReplaceAppUI
    ...
    % $_$
    % $_$ add parameter to creator method
    mymcode=uif_createComponents_switcher(mymcode, appname);
end
...
propblockstart = find(~cellfun('isempty',regexp(...
                                                mymcode, '^\s*properties', 'start'...
                                            )...
                               )...
                       );
                   
endstatements = find(~cellfun('isempty', regexp(...
                                                mymcode, '^\s*end', 'start'...
                                                )...
                               )...
                     );
...
if ~isempty(propblockstart)
    ...
    for ii = 1:npropblocks
        ...
        tmpblock = mymcode(propertyblockpair(ii,1)+ 1 ...
                          : propertyblockpair(ii,2)- 1);
        %$_$ commented out              
